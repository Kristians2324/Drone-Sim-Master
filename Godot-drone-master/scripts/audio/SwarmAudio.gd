...
var user_volume_db: float = -26.0 # Soft, non-intrusive background default
...
func setup_swarm_audio() -> void:
	...
	if swarm_player_3d == null:
		...
		swarm_player_3d.unit_size = 12.0
		swarm_player_3d.max_distance = 300.0
...
func update_swarm_audio(center_pos: Vector3, avg_speed: float, count: int, delta: float = 0.016) -> void:
	...
	if swarm_player_3d:
		...
		# Soft, balanced volume scaling that stays below background music
		var log_count_boost = log(float(max(count, 1))) / log(2.0) * 1.5
		var base_vol = user_volume_db - 4.0
		var speed_vol_boost = clampf(avg_speed / 20.0, 0.0, 1.0) * 2.0
		var target_vol = clampf(base_vol + log_count_boost + speed_vol_boost, -45.0, -22.0) # Soft max cap at -22 dB!

		var target_pitch = clampf(0.95 + (avg_speed / 30.0) * 0.15, 0.90, 1.20)

		var dt = clampf(delta * 6.0, 0.05, 1.0)
		var cur_vol = swarm_player_3d.volume_db if not is_nan(swarm_player_3d.volume_db) and not is_inf(swarm_player_3d.volume_db) else -35.0
...
## Generates a smooth, warm distant hive hum (2 seconds loop, zero crackle)
static func build_swarm_buzz_wav() -> AudioStreamWAV:
	...
	var f0 = 110.0 # Warm, smooth A2 fundamental pitch
	...
	for i in range(num_samples):
		...
		# Smooth multi-oscillator sine blend
		var s0 = sin(TAU * f0 * t) * 0.35
		var s1 = sin(TAU * (f0 * 1.008) * t + 0.3) * 0.25
		var s2 = sin(TAU * (f0 * 0.992) * t + 0.7) * 0.25
		var sub = sin(TAU * 55.0 * t) * 0.15

		# Soft gentle swell modulation (no harsh chopping!)
		var swell = 0.92 + 0.08 * sin(TAU * 2.5 * t)

		var raw_l = (s0 + s1 + sub) * swell * 0.22
		var raw_r = (s0 + s2 + sub) * swell * 0.22

		# 1-pole lowpass filter smoothing to eliminate crackle/clicks
		prev_l = lerpf(prev_l, raw_l, 0.25)
		prev_r = lerpf(prev_r, raw_r, 0.25)
