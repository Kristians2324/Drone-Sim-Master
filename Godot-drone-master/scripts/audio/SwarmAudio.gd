...
## Eliminates constructive sound stacking, preventing deafening audio clipping.
...
var user_volume_db: float = -14.0 # Soft background default
...
func setup_swarm_audio() -> void:
	...
	if swarm_player_3d == null:
		...
		swarm_player_3d.unit_size = 20.0
		swarm_player_3d.max_distance = 600.0
...
func update_swarm_audio(center_pos: Vector3, avg_speed: float, count: int, delta: float = 0.016) -> void:
	...
	if swarm_player_3d:
		...
		# Logarithmic volume scaling prevents deafening sound stacking regardless of drone count
		var log_count_boost = log(float(max(count, 1))) / log(2.0) * 2.5 # ~ 2.5 dB per doubling
		var base_vol = user_volume_db - 4.0
		var speed_vol_boost = clampf(avg_speed / 20.0, 0.0, 1.0) * 3.5
		var target_vol = clampf(base_vol + log_count_boost + speed_vol_boost, -40.0, -8.0) # Strictly capped at -8.0 dB max!

		# Pitch scale shifts softly with swarm velocity (1.00 base up to 1.30 pitch)
		var target_pitch = clampf(0.95 + (avg_speed / 25.0) * 0.30, 0.85, 1.45)

		var dt = clampf(delta * 8.0, 0.05, 1.0)
		var cur_vol = swarm_player_3d.volume_db if not is_nan(swarm_player_3d.volume_db) and not is_inf(swarm_player_3d.volume_db) else -30.0
		...
		swarm_player_3d.pitch_scale = clampf(lerpf(cur_pitch, target_pitch, dt), 0.5, 2.5)

## Generates a warm, multi-layered distant hive buzz stream (2 seconds loop)
static func build_swarm_buzz_wav() -> AudioStreamWAV:
	...
	var f0 = 128.0 # Base swarm drone fundamental frequency

	for i in range(num_samples):
		var t = float(i) / 44100.0

		# Multi-oscillator chorus representing flock density
		var s0 = sin(TAU * f0 * t) * 0.35
		var s1 = sin(TAU * (f0 * 1.015) * t + 0.4) * 0.25
		var s2 = sin(TAU * (f0 * 0.985) * t + 0.8) * 0.25
		var s3 = sin(TAU * (f0 * 2.02) * t + 1.2) * 0.15
		var s4 = sin(TAU * (f0 * 3.01) * t + 1.6) * 0.10

		# Soft collective modulation hum
		var swarm_chop = 0.88 + 0.12 * sin(TAU * 12.0 * t + sin(TAU * 1.5 * t) * 2.0)
		var air_rumble = sin(TAU * 65.0 * t) * 0.15

		var sample_l = (s0 + s1 + s3 + air_rumble) * swarm_chop * 0.45
		var sample_r = (s0 + s2 + s4 + air_rumble) * swarm_chop * 0.45

		var int_l = int(clampf(sample_l, -0.95, 0.95) * 32767.0)
		var int_r = int(clampf(sample_r, -0.95, 0.95) * 32767.0)
